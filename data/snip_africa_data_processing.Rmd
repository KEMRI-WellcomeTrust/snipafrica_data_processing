...
```{r}
#Entry
entry_data <- snip_data %>% filter(redcap_event_name == "Entry (Arm 1: Arm 1)")
# Replace "Unchecked" with NA in all factor columns
entry_data <- entry_data[, colSums(!is.na(entry_data)) > 0]

#Deterioration
det_data <- snip_data %>% filter(redcap_event_name == "Deterioration and New Episode (Arm 1: Arm 1)")
# Replace "Unchecked" with NA in all factor columns
det_data <- det_data[, colSums(!is.na(det_data)) > 0]

#Exit
exit_data <- snip_data %>% filter(redcap_event_name == "Exit (Arm 1: Arm 1)")
# Replace "Unchecked" with NA in all factor columns
